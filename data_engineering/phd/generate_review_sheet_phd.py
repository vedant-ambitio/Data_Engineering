#!/usr/bin/env python3
"""
generate_review_sheet_phd.py — Excel review sheet for PhD low confidence courses
==================================================================================

Sheet 1 (Review): Key fields extracted via regex from markdown text (PhD-specific)
...
MD_DIR = "classification_results/phd/low_confidence"
OUTPUT_FILE = "phd_review_low_v2.xlsx"
...
def extract_tuition(md):
    ...
    section = _section(md, ["Tuition", "Fees", "Cost", "Funding"])
    if not section:
        section = md
    ...
    patterns = [
        ...
        r'(?:tuition|fee)[^\n]*?[\$£€][\s]?(\d{1,3}(?:,\d{3})*(?:\.\d+)?)',
        r'(?:USD|GBP|EUR|CAD|AUD|DKK)\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)',
    ]
    found = []
    for pat in patterns:
    ...
    # Check for "fully funded" / "tuition waiver"
    if "fully funded" in section.lower() or "tuition waiver" in section.lower() or "free for" in section.lower():
        if not found:
            return "Fully funded / Tuition waived"
    ...
    if found:
        ...
        elif "DKK" in ctx.upper():
            return f"DKK {val:,.0f}/year"
        else:
            return f"USD {val:,.0f}/year"
    return None


def extract_stipend(md):
    """Find PhD stipend amount."""
    section = _section(md, ["Funding", "Stipend", "Cost"])
    if not section:
        return None

    # Patterns for stipend amounts
    patterns = [
        r'stipend[^.\n]*?[\$£€][\s]?(\d{1,3}(?:,\d{3})*(?:\.\d+)?)',
        r'[\$£€][\s]?(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(?:per\s+year|annually|stipend|salary)',
        r'(?:USD|GBP|EUR|CAD|AUD|DKK)\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(?:per\s+year|annually|monthly|per\s+month)',
        r'stipend[^.\n]*?(?:USD|GBP|EUR|DKK)\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)',
    ]
    for pat in patterns:
            ...
            try:
                ...
                if 500 < val < 100000:
                    ...
                    if "£" in ctx or "GBP" in ctx.upper():
                        ...
                        cur = "USD"
                    period = "month" if "month" in ctx.lower() else "year"
                    return f"{cur} {val:,.0f}/{period}"
            except ValueError:
                continue
    return None


def extract_funded_status(md):
    """Detect if PhD is fully funded."""
    section = _section(md, ["Funding", "Stipend"])
    if not section:
        section = md
    text = section.lower()
    if "fully funded" in text or "fully-funded" in text:
        return "Fully funded"
    if "partially funded" in text or "partial funding" in text:
        return "Partially funded"
    if "not funded" in text or "self-funded" in text or "self funded" in text:
        return "Self-funded / Not funded"
    if "tuition waiver" in text or "tuition remission" in text:
        return "Tuition waived"
    return None


def extract_duration(md):
    """Find PhD duration."""
    section = _section(md, ["Program Overview", "Duration", "Program Structure"])
    if not section:
        section = md

    patterns = [
        r'duration[:\s]+(\d+)\s*[-+]?\s*(?:to\s+\d+\s+)?years?',
        r'(\d+)\s*[-+]?\s*(?:to\s+\d+\s+)?years?\s+(?:full[-\s]time|PhD|program|fellowship)',
...
def extract_test_score(md, test_name):
    """Extract minimum score for TOEFL/IELTS/Duolingo/PTE/Cambridge."""
    ...
    elif test_name.upper() == "CAMBRIDGE":
        patterns = [
        ...
        if m:
            ...
            try:
                ...
                if test_name.upper() == "PTE" and not (30 <= num <= 90):
                    continue
                if test_name.upper() == "CAMBRIDGE" and not (140 <= num <= 230):
                    continue
            except ValueError:
                continue
            return val
...
def extract_work_experience(md):
    """Find work/research experience requirements."""
    section = _section(md, ["Work Experience", "Research Experience", "Admission Requirements", "Eligibility"])
    if not section:
        section = md

    patterns = [
        r'(\d+)\s*[-+]?\s*(?:to\s+\d+\s+)?years?\s+(?:of\s+)?(?:professional\s+|research\s+|relevant\s+|work\s+)experience',
        r'(?:work|research|professional)\s+experience[^.\n]*?(\d+)\s*[-+]?\s*years?',
    ]
    for pat in patterns:
        m = re.search(pat, section, re.IGNORECASE)
        if m:
    ...
    we_text = re.search(r'(?:work|research)\s+experience[^.\n]{0,150}', section, re.IGNORECASE)
    if we_text:
...
def extract_gpa(md):
    ...
    patterns = [
        ...
        r'(\d:\d)\s*(?:Bachelors?|Honours?|degree)',
    ]
    for pat in patterns:
        m = re.search(pat, section, re.IGNORECASE)
        if m:
...
def extract_research_areas(md):
    """Find research areas / focus."""
    section = _section(md, ["Faculty & Research", "Research Areas", "Research", "Program Overview"])
    if not section:
        return None

    # Look for bullet-listed research areas
    ...
    if bullets:
        ...
        for b in bullets:
            b = b.strip().rstrip(":")
            if b and b.lower() not in seen and len(b) < 80:
                seen.add(b.lower())
                result.append(b)
            if len(result) >= 5:
                break
...
def extract_supervisor_required(md):
    """Detect if supervisor pre-approval is required."""
    section = _section(md, ["Faculty & Research", "Application", "Admission Requirements", "Program Structure"])
    if not section:
        section = md
    text = section.lower()
    if "reach out to potential supervisors" in text or "contact potential supervisors" in text:
        return "Yes — contact supervisor before applying"
    if "supervisor agreement" in text or "supervisor approval" in text:
        return "Yes — supervisor approval required"
    if "advisor matching" in text or "matched with" in text:
...
def extract_official_link(md):
    ...
    priority = ["phd", "doctoral", "graduate", "program", "admission"]
    for url in urls:
        url_lower = url.lower()
...
def extract_deadlines_by_intake(md):
    """Extract deadlines split by intake."""
    ...
    for i, m in enumerate(matches):
        ...
        year_m = re.search(r'20\d{2}', m.group(0))
...
def extract_fields_from_md(md):
    """Extract all PhD-specific fields from markdown text via regex."""
    if not md:
        return {}
    ...
    used_keys = {"summer_2026", "fall_2026", "winter_2026", "spring_2027",
                 "summer_2027", "fall_2027", "winter_2027"}
    ...
    return {
        ...
        "funding_status": extract_funded_status(md_clean),
        "stipend": extract_stipend(md_clean),
        "duration": extract_duration(md_clean),
        "GRE_required": extract_test_required(md_clean, "GRE"),
        "GRE_score": extract_gre_score(md_clean),
        "GPA_min": extract_gpa(md_clean),
        "research_experience": extract_work_experience(md_clean),
        "LOR_count": extract_lor_count(md_clean),
        "supervisor_required": extract_supervisor_required(md_clean),
        "research_areas": extract_research_areas(md_clean),
        ...
        "Cambridge_min": extract_test_score(md_clean, "CAMBRIDGE"),
        "officialPageLink": extract_official_link(md),
        "deadline_summer_2026": deadline_parts.get("summer_2026", ""),
        "deadline_fall_2026": deadline_parts.get("fall_2026", ""),
        "deadline_winter_2026": deadline_parts.get("winter_2026", ""),
        "deadline_spring_2027": deadline_parts.get("spring_2027", ""),
        "deadline_summer_2027": deadline_parts.get("summer_2027", ""),
        "deadline_fall_2027": deadline_parts.get("fall_2027", ""),
        "deadline_winter_2027": deadline_parts.get("winter_2027", ""),
        "deadline_other": "; ".join(other_parts) if other_parts else "",
    }
...
def main():
    ...
    headers = [
        ...
        "stipend", "duration",
        "GRE_required", "GRE_score",
        "GPA_min", "research_experience", "LOR_count",
        "supervisor_required",
        "TOEFL_min", "IELTS_min", "Duolingo_min", "PTE_min", "Cambridge_min",
        "officialPageLink",
        "confidence_tier",
