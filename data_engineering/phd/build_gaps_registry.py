"""
build_gaps_registry.py  (PhD edition)

Builds the per-course PhD gaps registry from two existing sources:

  1. classification_results/phd/classification_report.json
     -> per-course statuses for the 3 classified sections (tuition_and_fees,
        application_deadlines, admission_requirements) and the entities
        inside admission_requirements (TOEFL, IELTS, GPA, GRE, etc.)
     Gap statuses we treat as needing re-verification (medium severity):
       - section status: "flagged", "partially_verified"
       - entity status:  "mismatched", "not_in_crawled"

  2. phd_data/phd/<college>/<file>.md
     -> any <citation> block with `status: missing`
        (PhD MDs have ZERO status:missing blocks in practice — all signal
        comes from the classification report — but the parser stays for
        parity with the masters pipeline.)

Output (in phd_v2/gaps/):
...
Usage:
    python phd_v2/scripts/build_gaps_registry.py
    python phd_v2/scripts/build_gaps_registry.py --limit 10   (stratified pilot)
"""
...
MD_DIR = BASE / "phd_data" / "phd"
OUT_DIR = BASE / "phd_v2" / "gaps"

# Only treat report-detected CONFLICTS as gaps. `no_data` (the report had no
# crawled URLs to compare against) is NOT a conflict — the MD might still have
# perfectly good information from sources the crawler didn't see. Re-verifying
# those would waste API calls just to overwrite MD content with a fresh search
# of the same kind of data. Same set as the masters pipeline.
GAP_SECTION_STATUSES = {"flagged", "partially_verified"}
GAP_ENTITY_STATUSES = {"mismatched", "not_in_crawled"}
...
def extract_report_gaps(r):
    """From one classification_report result, return list of section + entity gaps.

    admission_requirements is intentionally NOT emitted as a section-level gap:
    its section status is purely a roll-up of entity statuses (every "flagged"
    reason follows the pattern `X_matched_Y_mismatched`). The entity-level gaps
    below already cover every issue, so we skip the section to avoid duplicate
    work and to keep Gemini prompts focused on the specific mismatched entity.
    """
    gaps = []
    ...
    for ent_name, ent_data in adm_details.get("entities", {}).items():
        status = ent_data.get("status", "")
        if status in GAP_ENTITY_STATUSES:
            gaps.append({
                "type": "entity",
                "field": ent_name,
                "section": "admission_requirements",
                "status": status,
                "md_value": ent_data.get("md_value", ent_data.get("md_values")),
                "crawled_value": ent_data.get("crawled_value"),
            })
    return gaps
...
def main():
    parser = argparse.ArgumentParser(description="Build per-course gaps registry for masters re-verification.")
    ...
    for i, r in enumerate(results):
        ...
        for g in report_gaps:
            if g["type"] == "section":
                ...
                stats["report_entity_gaps"][f"{g['field']} :: {g['status']}"] += 1
        for g in md_gaps:
    ...
    for k in ("report_section_gaps", "report_entity_gaps", "md_section_gaps",
    ...
    print("\nReport ENTITY gaps (top 10):")
    for k, v in sorted(stats["report_entity_gaps"].items(), key=lambda x: -x[1])[:10]:
        print(f"  {k:<50s} {v:>6,}")
