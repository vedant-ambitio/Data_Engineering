"""Stage 1 (Tier B): Unwrap vertex grounding redirects for Tier B universities.

Source: grounding_tier_b/*.json
Cache:  unwrap_cache_tier_b.json
"""
import json
import os
import time
...
CACHE_PATH = os.path.join(BASE, "unwrap_cache_tier_b.json")

CONCURRENCY = 100
TIMEOUT = 8

print("Scanning grounding_tier_b/ for chunk URIs...")
all_uris = set()
...
print(f"Stage 1 (Tier B): unwrapping with {CONCURRENCY} workers (verify=False, timeout={TIMEOUT}s)")
start = time.time()
done = 0
last_save = 0
SAVE_EVERY = 500
