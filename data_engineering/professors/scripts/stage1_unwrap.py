"""Stage 1: Unwrap vertex grounding redirects.

For every chunk URI in every grounding/*.json:
  - HTTP HEAD allow_redirects=False, verify=False
  - Read the 'Location' header → real URL
  - Cache to unwrap_cache.json

Resumable: re-running picks up from cache.
"""
import json
import os
import time
...
# Shared session with a connection pool large enough for our workers.
...
CACHE_PATH = os.path.join(BASE, "unwrap_cache.json")

CONCURRENCY = 100
TIMEOUT = 8
USER_AGENT = "Mozilla/5.0 (compatible; CoverageBot/1.0)"

# Pass 1: collect all unique vertex URIs
print("Scanning grounding/ for chunk URIs...")
all_uris = set()
...
print(f"Stage 1: unwrapping with {CONCURRENCY} workers (verify=False, timeout={TIMEOUT}s)")
start = time.time()
done = 0
last_save = 0
SAVE_EVERY = 500
