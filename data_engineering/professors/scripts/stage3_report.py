"""Stage 3: classify each (uni, dept) using unwrap + probe caches and write Excel.

For each grounding file:
  - For each chunk, look up unwrap_cache[chunk_uri] -> real_url
  - Check is_subdomain_match(real_url, university's official domain)
  - Look up probe_cache[real_url] -> working?
  - FOUND if any chunk is BOTH on official domain AND probed working
  - Else NOT_FOUND

Output: coverage_grounding_v3.xlsx with same 5-column layout as v2.
"""
import csv
import json
import os
...
UNWRAP_CACHE = os.path.join(BASE, "unwrap_cache.json")
PROBE_CACHE = os.path.join(BASE, "probe_cache.json")
...
# Original confidences from output/universities/ — needed to identify
# (uni, dept) pairs where the department doesn't exist at the university.
print("Loading original confidences...")
orig_conf = {}  # (uni, dept) -> {"confidence": ..., "pre_filtered": ...}
for fn in os.listdir(OUTPUT_UNIS):
    if not fn.endswith(".json"):
        continue
    with open(os.path.join(OUTPUT_UNIS, fn), "r", encoding="utf-8") as f:
        odata = json.load(f)
    for d, e in odata.get("departments", {}).items():
        orig_conf[(odata.get("university", ""), d)] = {
            "confidence": e.get("confidence"),
            "pre_filtered": e.get("pre_filtered", False),
        }


def dept_absent(uni, dept):
    """True if the dept doesn't exist at the uni (per original pipeline)."""
    info = orig_conf.get((uni, dept))
    if not info:
        return False
    return (info["confidence"] in ("not_found", "not_applicable")
            or bool(info["pre_filtered"]))
...
for fn in files:
    ...
    for c in chunks:
        ...
        if not real_url:
            # fall back to chunk's domain field if unwrap failed (rare)
...
ws_sum.append(["Coverage v3 — every chunk URL HTTP-probed (Tier A)"])
