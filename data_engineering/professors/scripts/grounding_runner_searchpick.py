#!/usr/bin/env python3
"""
grounding_runner_searchpick.py — Tier B faculty URL discovery, search-pick mode.
...
OUTPUTS — structured to mirror Professors_info/output/ for easy merging later.
  ...
  output_tier_b/faculty_urls.csv
  output_tier_b/faculty_urls.jsonl
  output_tier_b/universities/<uni>.json
  grounding_tier_b/<uni>__<dept>.json    (raw per-pair audit dump)
  logs/state_tier_b.jsonl                 (resumable state log)

USAGE
  python Professors_info/scripts/grounding_runner_searchpick.py --pilot -v
      → first 5 Tier B unis (testing)
  python Professors_info/scripts/grounding_runner_searchpick.py --all -v
      → all 151 Tier B unis x 30 depts (4,530 pairs)
  python Professors_info/scripts/grounding_runner_searchpick.py --all --skip-existing -v
      → resume (skip pairs already in state_tier_b.jsonl)
  python Professors_info/scripts/grounding_runner_searchpick.py --dry-run --pilot
...
OUTPUT_DIR = PROJECT_ROOT / "output_tier_b"
UNIS_DIR = OUTPUT_DIR / "universities"
GROUNDING_DIR = PROJECT_ROOT / "grounding_tier_b"
LOGS_DIR = PROJECT_ROOT / "logs"
for d in (OUTPUT_DIR, UNIS_DIR, GROUNDING_DIR, LOGS_DIR):
    d.mkdir(parents=True, exist_ok=True)

UNIVERSITIES_CSV = CONFIG_DIR / "universities_tier_B.csv"
DEPARTMENTS_CSV = CONFIG_DIR / "top_30_departments_enriched.csv"
STATE_FILE = LOGS_DIR / "state_tier_b.jsonl"
...
PILOT_FIRST_N_UNIS = 5    # --pilot processes first N Tier B unis (alphabetical)

log = logging.getLogger("searchpick")
...
# ══════════════════════════════════════════════════════════════════════════════
#  AGGREGATION → output_tier_b/{faculty_urls.csv,jsonl,universities/}
# ══════════════════════════════════════════════════════════════════════════════
def rebuild_outputs():
...
def main():
    p = argparse.ArgumentParser(description="Tier B faculty URL discovery (search-pick mode)")
    p.add_argument("--pilot", action="store_true", help=f"Run only first {PILOT_FIRST_N_UNIS} Tier B unis (testing)")
    p.add_argument("--all", action="store_true", help="Run all 151 Tier B unis x 30 depts")
    ...
    if not UNIVERSITIES_CSV.exists():
        log.error("Tier B CSV not found: %s", UNIVERSITIES_CSV)
        sys.exit(1)
