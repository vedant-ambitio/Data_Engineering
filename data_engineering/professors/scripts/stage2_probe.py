"""Stage 2: Probe each unique real URL from Stage 1.

For every unique real URL in unwrap_cache.json:
  - HTTP HEAD allow_redirects=True, verify=False, 8s timeout
  - If HEAD fails (405/403/etc.), fall back to GET stream=True
  - Cache to probe_cache.json: {real_url -> {status, ok, final_url}}

Resumable: re-running picks up from probe_cache.json.
"""
import json
import os
import time
...
# Shared session (large connection pool — destination URLs span ~10k different hosts)
...
# Load Stage 1 cache
print("Loading unwrap_cache.json...")
...
print(f"Stage 2: probing with {CONCURRENCY} workers (verify=False, timeout={TIMEOUT}s)")
start = time.time()
done = 0
last_save = 0
SAVE_EVERY = 500
