"""Stage 3 (Tier B): classify each (uni, dept) using unwrap + probe caches and write Excel.

Source:
  grounding_tier_b/         (4,529 grounding files)
  output_tier_b/universities/   (151 university JSONs with original confidences)
  unwrap_cache_tier_b.json
  probe_cache_tier_b.json
  config/universities_tier_B.csv

Output: coverage_grounding_v3_tier_b.xlsx
"""
import csv
import json
import os
...
GROUNDING = os.path.join(BASE, "grounding_tier_b")
STATE_LOG = os.path.join(BASE, "logs", "state_tier_b.jsonl")
TIER_CSV = os.path.join(BASE, "config", "universities_tier_B.csv")
UNWRAP_CACHE = os.path.join(BASE, "unwrap_cache_tier_b.json")
PROBE_CACHE = os.path.join(BASE, "probe_cache_tier_b.json")
OUT_XLSX = os.path.join(BASE, "coverage_grounding_v3_tier_b.xlsx")
...
print(f"Tier B universities in CSV: {len(tier)}")
...
# Original confidences from logs/state_tier_b.jsonl (Tier B has no per-uni
# JSON with confidence; the log carries it).  Keep the LATEST entry per
# (uni, dept) in case the pipeline retried.
print("Loading original confidences from state_tier_b.jsonl...")
orig_conf = {}
...
def dept_absent(uni, dept):
    """True if the department doesn't exist at the university (per Tier B log).
    Tier B uses confidence='not_found' for these (no pre_filtered field)."""
...
ws_sum.append(["Coverage v3 — Tier B (per-chunk HTTP probed)"])
...
headers = ["Department", "Tier B universities", "FOUND (official + working)",
