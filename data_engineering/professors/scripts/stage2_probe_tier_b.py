"""Stage 2 (Tier B): Probe each unique real URL from Tier B Stage 1.

Source: unwrap_cache_tier_b.json
Cache:  probe_cache_tier_b.json

For every unique real URL:
  - HTTP HEAD allow_redirects=True, verify=False, 5s timeout
  - If HEAD fails (405/403/etc.), fall back to GET stream=True
  - Cache: {real_url -> {status, ok, final_url}}

Resumable: re-running picks up from probe_cache_tier_b.json.
"""
import json
import os
import time
...
print("Loading unwrap_cache_tier_b.json...")
...
print(f"Stage 2 (Tier B): probing with {CONCURRENCY} workers (verify=False, timeout={TIMEOUT}s)")
start = time.time()
done = 0
last_save = 0
SAVE_EVERY = 500
