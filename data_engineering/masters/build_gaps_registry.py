"""
build_gaps_registry.py

Builds the per-course gaps registry from two existing sources:

  1. classification_results/masters/classification_report.json
     -> per-course statuses for the 3 classified sections (tuition_and_fees,
        application_deadlines, admission_requirements) and the 11 entities
  ...
  2. masters_data/masters/<college>/<file>.md
     -> any <citation> block with `status: missing`
        (covers Curriculum, Scholarships, Class Profile, Career Outcomes,
        Contact Information, and any other section not in the report)

Output (in masters_v2/gaps/):
...
Usage:
    python masters_v2/scripts/build_gaps_registry.py
"""
