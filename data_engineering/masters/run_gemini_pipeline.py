...
Reads a gaps registry (pilot or full), and for each gap calls Gemini 3.0 Flash
with Google Search grounding. Writes:
  ...
  - masters_v2/pipeline_state/cost_log.jsonl
        One line per Gemini call: timestamp, course, field, tokens, cost.

The script is resumable: if an evidence file already exists, the course is
skipped (override with --force).

Usage:
    # Pilot run (10 courses):
    python masters_v2/scripts/run_gemini_pipeline.py --registry gaps_registry_pilot_10.json

    # Full run (all 11k):
    python masters_v2/scripts/run_gemini_pipeline.py --registry gaps_registry.json

    # Override model:
    python masters_v2/scripts/run_gemini_pipeline.py --registry gaps_registry_pilot_10.json --model gemini-2.5-flash

    # Force re-process even if evidence exists:
    python masters_v2/scripts/run_gemini_pipeline.py --registry gaps_registry_pilot_10.json --force
"""
...
def build_section_prompt(field, program, university, gap):
    ...
    if field == "tuition_and_fees":
        return (
            f"Find the CURRENT tuition fees for the \"{program}\" master's program at \"{university}\".\n\n"
            ...
            f"    *   **International Students (Non-EU/EEA):** <amount per year, plus total program cost if available>\n"
            f"    *   **Domestic / EU / EEA Students:** <amount or 'No tuition fees'>\n"
            f"*   **Application Fee:** <amount with currency, or 'No application fee'>\n"
            f"*   **Living Expenses:** <estimated annual or monthly cost of living>\n"
            f"*   **Scholarships / Fee Waivers:** <brief list of named scholarships or 'Information not available'>\n"
            f"*   **Additional Fees:** <any other charges, or 'Information not available'>\n\n"
            f"Example of good output:\n"
            f"*   **Tuition Rates (Academic Year 2026-2027):**\n"
            f"    *   **International Students (Non-EU/EEA):** 115,700 DKK per year (approximately 15,490 EUR). Total 2-year program: 231,400 DKK.\n"
            f"    *   **Domestic / EU / EEA Students:** No tuition fees.\n"
            f"*   **Application Fee:** 150 EUR (approximately 1,150 DKK). Required for non-EU/EEA citizens.\n"
            f"*   **Living Expenses:** Approximately 96,000-138,000 DKK per year.\n"
            f"*   **Scholarships / Fee Waivers:** Danish Government Scholarships available for non-EU/EEA students (full tuition waiver + monthly stipend).\n"
    ...
    if field == "application_deadlines":
        return (
            f"Find the CURRENT application deadlines for the \"{program}\" master's program at \"{university}\".\n\n"
            f"List ALL intakes (Fall, Spring, Summer) with their specific deadline dates for the "
            f"upcoming admission cycle. Include early action / regular / late rounds if applicable. "
            f"Mention rolling admissions if that's how the program operates. "
            f"Note any program-specific deadlines that differ from the university default."
            f"{hint}{source_directive}\n\n"
            f"Format your answer as a Markdown bulleted list. Use this exact structure:\n\n"
            f"*   **<Intake Name> (<Year> Intake):** <Deadline date OR 'Rolling admissions, no set deadline'> "
            f"<optional one-sentence context>\n"
            f"    *   <optional nested bullet with sub-deadline or applicant-type difference>\n\n"
            f"Examples of good format:\n"
            f"*   **Fall 2026 (September Intake):** Rolling admissions, no set deadline; early applications recommended.\n"
            f"*   **Spring 2027 (January Intake):** Application deadline: November 15, 2026.\n"
            f"    *   International applicants: October 1, 2026, to allow time for visa processing.\n\n"
            f"Rules:\n"
            f"- Do NOT include URLs in your response. Do NOT add 'Source:' lines.\n"
            f"- Do NOT add introductory or closing paragraphs — bullets only.\n"
            f"- If no upcoming-cycle dates are available, state that under the intake bullet."
        )

    # Fallback for any other section
    return (
        f"Find current information about \"{field}\" for the \"{program}\" master's program "
...
ENTITY_QUESTIONS = {
    ...
    "gpa":             "What is the minimum GPA required for admission to \"{program}\" at \"{university}\"? Provide value with scale (e.g. 3.0/4.0).",
    "gre_status":      "Is the GRE required, optional, or waived for admission to \"{program}\" at \"{university}\"?",
    "gre_score":       "What is the minimum or recommended GRE score for admission to \"{program}\" at \"{university}\"?",
    ...
    "sop":             "Is a Statement of Purpose required for admission to \"{program}\" at \"{university}\"? Any length/word limit?",
}
...
def build_md_missing_prompt(course, gap):
    ...
    return (
        f"Find information about \"{section}\" for the \"{program}\" master's program at \"{university}\".\n\n"
...
def main():
    ...
    parser.add_argument("--registry", default="gaps_registry_pilot_10.json",
                        help="Registry filename in masters_v2/gaps/")
