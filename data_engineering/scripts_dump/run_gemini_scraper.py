...
def gemini_web_search(prompt, max_tokens=MAX_OUTPUT_TOKENS, temperature=0.3, attempt=0):
    ...
    return {"text": text, "sources": sources, "supports": supports, "_chunk_urls": chunk_urls}
...
def resolve_redirect(url):
    """Resolve vertexaisearch redirect URLs to actual URLs via HEAD/GET request."""
    ...
    # Method 1: curl follow redirect with -w redirect_url
    try:
        result = subprocess.run(
            ["curl", "-sI", "-o", "/dev/null", "-w", "%{redirect_url}", "--max-time", "8", "-L", url],
            capture_output=True, text=True, timeout=15,
        )
        resolved = result.stdout.strip()
        if resolved and resolved.startswith("http") and "vertexaisearch" not in resolved:
            with _url_cache_lock:
    ...
    # Method 2: curl follow redirect and capture final URL with -w url_effective
    try:
        result = subprocess.run(
            ["curl", "-s", "-o", "/dev/null", "-w", "%{url_effective}", "--max-time", "8", "-L", url],
            capture_output=True, text=True, timeout=15,
        )
        resolved = result.stdout.strip()
        if resolved and resolved.startswith("http") and "vertexaisearch" not in resolved:
    ...
    # Method 3: curl get Location header directly
    try:
        result = subprocess.run(
...
def build_prompt_ug(university, country, program):
    """Build a comprehensive prompt for undergraduate/bachelor's program research."""
    program_name = program["program_name"]
    degree = program.get("degree", "")
    url = program.get("url", "")
    tags = program.get("field_tags", [])
    ...
    return f"""You are an undergraduate program research expert. Produce a comprehensive markdown document about this undergraduate/bachelor's program.
...
RESEARCH INSTRUCTIONS:
You MUST call the search tool MANY times — at least 20-23 separate searches. Do NOT stop after a few searches.{f" START by searching for the OFFICIAL URL and ADDITIONAL KNOWN URLS above — these are verified official pages. Extract data DIRECTLY from these official pages first." if extra_urls else ""} Perform SEPARATE dedicated searches for EACH of these:
1. "{university} {degree} {program_name}" — the program's main page
2. "{university} {degree} {program_name} admission requirements" — FIRST search the OFFICIAL admissions page to discover what this university actually requires. Different systems exist: US uses GPA/SAT/ACT, UK uses A-levels/IB, etc.
3. "{university} {degree} {program_name} application deadlines {year}" — all rounds, priority, early, final, early decision, early action
4. "{university} {degree} {program_name} tuition fees {year}" — per-credit, per-semester, total cost, in-state vs out-of-state vs international
5. "{university} undergraduate admissions SAT ACT score requirements {year}" — ONLY search this if search #2 confirmed SAT/ACT are required or accepted. If this university does not use SAT/ACT (e.g., UK, most non-US), SKIP this search entirely.
6. "{university} {degree} {program_name} TOEFL IELTS Duolingo English requirements" — ONLY search for tests the official admissions page actually lists. If the official page specifies Standard vs Higher level, note which level applies to THIS program.
7. "{university} {degree} {program_name} scholarships financial aid" — merit, need-based, grants
8. "{university} {degree} {program_name} curriculum courses" — course list, major requirements, electives
9. "{university} international students credential evaluation" — credential requirements for international applicants
9b. "{university} admission requirements Indian students" — search the OFFICIAL website for India-specific entry requirements (Class 12 percentages, accepted Indian qualifications)
10. "{university} {degree} {program_name} apply now application portal" — direct application link (Common App, Coalition, university portal)
11. "{university} {degree} {program_name} faculty professors" — notable faculty, department faculty page
12. "{university} undergraduate career outcomes employment salary" — placement data, starting salaries
13. "{university} {program_name} QS ranking THE ranking US News ranking {year}" — global and national rankings
14. "{university} {program_name} class profile acceptance rate admitted students {year}" — acceptance rate, class size, international %, enrolled students
15. "{university} {program_name} top recruiters employers hiring companies graduates" — recruiting companies and job roles
16. "{university} {program_name} average salary ROI return on investment graduates" — avg earnings, payback period, ROI data
17. "{university} {program_name} student life study load campus housing" — campus experience, housing, student organizations
18. "{university} {degree} {program_name} program duration credits semesters" — credit requirements, semester structure
19. "{university} {program_name} graduation rate completion rate retention rate {year}" — graduation %, retention %, completion rate
20. "{university} {program_name} graduate job titles roles positions career paths" — specific job titles graduates get (Data Analyst, Software Engineer, etc.)
21. "{university} {program_name} study load hours per week lectures contact hours" — weekly study hours, lecture hours, self-study expectations
22. "{university} {degree} {program_name} program duration years semesters full-time part-time length" — exact duration in years/semesters, full-time vs part-time options
23. "{university} {degree} {program_name} delivery mode on-campus online hybrid distance learning" — on-campus / online / hybrid, any remote learning options

OFFICIAL URL PRIORITY:
When OFFICIAL URL or ADDITIONAL KNOWN URLS are provided above, you MUST search those pages FIRST and extract data directly from them. These are verified official university pages. Only search elsewhere if the official pages do not contain the specific information needed.

DEEP EXTRACTION RULE:
When you land on an official university page that contains rich data, extract ALL relevant information from that page — not just the one thing you searched for. For example:
- If a tuition page also shows fees for different semesters (Fall, Winter, Spring, Summer), resident vs non-resident vs international — extract ALL of them in detail, not just one number.
- If a program page shows duration, delivery mode, credits, AND curriculum — extract everything in a descriptive way.
- If an admissions page shows requirements for multiple student types (domestic, international, transfer) — extract all variants.
- If a career page shows salary data, placement rates, AND recruiter names — extract all of it.
Write this data in a DESCRIPTIVE way with full context — not just bare numbers. Include which semester, which student type, which year the data applies to.

PRIORITY DATA — MUST BE ACCURATE (do follow-up searches if initial results are vague):
These fields are CRITICAL and will be verified against official sources. Get exact values, not approximations:

1. APPLICATION DEADLINES: Search for EVERY round and type separately. Must have exact dates (Month Day, Year).
   Include: Early Decision, Early Action, Regular Decision, Rolling deadlines.
   If you find "January 2026" without a day, do a follow-up search to find the exact day.
   Search: "{university} undergraduate application deadline early decision regular decision {year}"

2. TUITION & FEES: Must have exact dollar/currency amounts per year AND total program cost (4 years for bachelor's).
   Separate: in-state / out-of-state / international if applicable.
   Include: application fee, housing, meal plan, health insurance, student fees.
   Search: "{university} {program_name} tuition {year} in-state out-of-state international cost of attendance"

3. TOEFL SCORE: ONLY if the official admissions page lists TOEFL as accepted/required. Must have exact minimum overall score AND section minimums if published. If the official page specifies different levels (Standard vs Higher), report the correct level for THIS specific program.
   Search: "{university} undergraduate admissions TOEFL minimum score sections"

4. IELTS SCORE: ONLY if the official admissions page lists IELTS as accepted/required. Must have exact minimum overall band AND per-band minimums if published. If the official page specifies different levels (Standard vs Higher), report the correct level for THIS specific program.
   Search: "{university} undergraduate admissions IELTS minimum band score"

5. SAT SCORE: ONLY if the official admissions page lists SAT as required/recommended/accepted. If this university does not use SAT (e.g., UK, most non-US), write "Not required/accepted" and SKIP the search. Do NOT pull estimated scores from third-party sites.
   Separate: total score, Evidence-Based Reading and Writing, Math sections.
   Search: "{university} admitted students SAT score range middle 50 {year}"

6. ACT SCORE: ONLY if the official admissions page lists ACT as required/recommended/accepted. If not used, write "Not required/accepted" and SKIP the search.
   Search: "{university} admitted students ACT score range middle 50 {year}"

7. GPA REQUIREMENT: ONLY if this university uses GPA (typically US universities). If they use A-level grades, IB points, Class 12 percentages, or ATAR, report THOSE instead — do NOT convert to GPA from third-party sites.
   Specify: unweighted vs weighted, scale (out of 4.0) if applicable.
   Search: "{university} undergraduate admitted students average GPA {year}"

8. LOR COUNT: Must state exact number of recommendation letters required.
   Note: teacher vs counselor requirements separately.
   Search: "{university} undergraduate application letters of recommendation how many required"

9. PROGRAM DURATION: Must state exact duration in years and total credits/units required.
   Search: "{university} {degree} {program_name} duration credits units degree requirements"

If ANY of these 9 priority fields returns vague data from the first search, you MUST do a SECOND dedicated search specifically for that field before writing "Information not available".

SOURCE PRIORITY (CRITICAL):
You MUST prioritize sources in this order — use official sources whenever possible:
1. BEST: Official university websites (.edu, .ac.uk, .ac, university-owned domains) — e-catalogue, admissions pages, department pages
2. OK: Government/accreditation databases (nces.ed.gov, hesa.ac.uk)
3. NEVER USE: ambitio.club, ambitio.in — do NOT use any data from ambitio websites
4. AVOID: Third-party aggregators — NEVER use collegedunia, shiksha, yocket, topuniversities, leverage.edu, masterstudies.com, masterscompare, lilacbuds, bschools.org, rightpath, myunisearch, gyandhan, mentr-me, gmat.edu.sg, mim-essay
5. LAST RESORT: Reputable third-party sources (clearadmit, usnews, qs) — only if official source is truly unavailable
...
1. NEVER fabricate or guess data. If you cannot find something, write "Information not available".
2. Include specific numbers, dates, and requirements — not vague statements.
3. For tuition, specify the academic year, per credit/semester/year/total, in-state vs out-of-state vs international.
4. For deadlines: today is {today}. Search HARD for upcoming relevant intakes: {deadline_hint}. Include specific dates (month and day) for EVERY round — Early Decision, Early Action, Regular Decision, international. Note if rolling admissions.
5. DO NOT include URLs or citation blocks — source URLs are tracked automatically via search grounding. Focus all output tokens on DATA. NEVER write "(Source: ...)" or any URL in your output.
6. IMPORTANT: Write each data point as a SEPARATE bullet point on its own line. Do NOT combine multiple facts into one long paragraph. Each deadline date, each tuition amount, each test score should be its own bullet. This helps with source tracking.
7. For SAT/ACT: ONLY if this university uses SAT/ACT, state if test-optional, test-required, or test-blind and include middle 50% ranges. If SAT/ACT are not part of this university's admission system, write "Not required/accepted for this program".
8. For credential evaluation: search what international credential evaluation services are accepted (WES, ECE, SpanTran, Scholaro, etc).
...
## University Overview
University type (public/private), location, brief institutional context.
- **Global Rankings**: QS World Ranking, Times Higher Education Ranking, US News Ranking — search and include specific rank numbers.

## Program Overview
What the program is, which school/department offers it. Include ALL key facts as bold fields:
- **Duration**: X years (X semesters) — must be specific
- **Total Credits**: X credits/units required to graduate
- **Degree Awarded**: Bachelor of Science / Bachelor of Arts / etc.
- **Department/School**: Name of school or college within the university
- **Delivery Mode**: On-campus / Online / Hybrid
- **Study Load**: Estimated hours per week (lectures + self-study)

## Curriculum
Detailed program structure organized by year/semester:
- Core courses (with course codes and credit hours if available)
- Major requirements and prerequisite chains
- Elective areas and number of elective credits
- Specializations/concentrations/tracks available
- Capstone/thesis/senior project requirements
- Minor options
- Organize with ### subsections by year/semester for clarity

## Admission Requirements
This is the most important section — be thorough. Cover ALL of:
- Educational background and prerequisites (high school diploma, specific subjects required like math/science)
- GPA requirements (minimum and/or average of admitted students, weighted vs unweighted)
- Standardized tests: SAT (required/optional/test-blind, middle 50% range, section scores), ACT (same)
- TEST POLICY: Is it test-optional, test-required, or test-blind? For which admission cycles?
- English proficiency: TOEFL iBT (total + section mins), IELTS (overall + section mins), Duolingo DET, PTE Academic — list ALL accepted tests with scores
- English proficiency waiver conditions (citizenship, English-medium school, etc.)
- Credential evaluation for international students: required? Which services accepted?
- Required documents with specifics: high school transcripts (Class 10 & 12), LOR count (teacher vs counselor), personal statement/essay (Common App essay, supplemental essays, word limits), CV/resume, portfolio if applicable
- Application platforms: Common App, Coalition App, QuestBridge, university's own portal
- Application fee (exact amount, currency, fee waiver availability and conditions)
- Interview: required/optional/not offered? Format (alumni, admissions officer, video)?

## Tuition & Fees
Use a TWO-STEP approach:

STEP 1 — DISCOVERY: First search the university's CENTRAL tuition/fees page to understand their fee structure:
- Does this university charge the SAME tuition for all undergraduate programs, or does it vary by program/department/faculty?
- What fee categories exist? (e.g., domestic vs international, in-state vs out-of-state, subsidized vs unsubsidized)
Search: "{university} undergraduate tuition fees {year}" (university-wide first, NOT program-specific)

STEP 2 — EXTRACT based on what you found:
- If tuition is UNIFORM across all UG programs (common for US private universities, many UK universities for same fee band): extract from the central tuition page — do NOT search for program-specific fees that don't exist.
- If tuition VARIES by program/faculty (common for UK universities across different departments, Australian universities, Singapore universities): search the SPECIFIC PROGRAM PAGE for this program's fees.
- If the university has MULTIPLE fee tiers by student type, extract ALL tiers:
  - US public: in-state / out-of-state / international
  - UK: Home / Overseas (and check if this program has Standard or Higher overseas fee)
  - Singapore: Singaporean / Permanent Resident / ASEAN / International, and subsidized (with tuition grant) vs unsubsidized
  - Australia: Commonwealth Supported Place (CSP) vs full-fee domestic vs international
  - Canada: domestic / international, and check for program-specific differential fees
  - For any other country: identify whatever domestic vs international fee structure applies

Extract these specific data points:
- Annual tuition amount (with currency and academic year it applies to)
- Total program cost (3 years for UK bachelor's, 4 years for US, etc.)
- Application fee amount
- Student services / activity / technology fees
- Health insurance cost (if mandatory)
- Estimated room & board / housing + food
- Estimated total cost of attendance per year (tuition + living expenses)
- Whether financial aid reduces the effective cost (e.g., "average net price after aid")

FALLBACK: If your first search finds no tuition data, try these additional searches:
- "{university} cost of attendance {year}"
- "{university} fees schedule undergraduate {year}"
- "{university} student finance tuition"

## Application Deadlines
Use a TWO-STEP approach:

STEP 1 — DISCOVERY: First search the OFFICIAL university admissions page to understand what application rounds exist. Different countries use completely different systems:
- US universities: may have Early Decision (ED, binding), Early Decision II (ED2), Early Action (EA, non-binding), Restrictive Early Action (REA), Regular Decision (RD), Rolling Admissions
- UK universities: use UCAS with a single equal consideration deadline (usually mid-January). Medicine/Oxbridge have an earlier October deadline. Some programs have additional aptitude tests (BMAT, UCAT, MAT, TMUA, etc.) with separate registration deadlines.
- Singapore universities (NUS, NTU): have multiple windows — direct admission, aptitude-based admission, early admission programme. Application typically opens in October-February.
- Australian universities: have multiple intake periods (February, July) with different closing dates per intake.
- Canadian universities: have program-specific deadlines that vary widely, often January-March.
Do NOT assume every university uses ED/EA/RD. Only include the rounds that actually exist for this university.

STEP 2 — EXTRACT for each round that exists:
- Application deadline (exact date: Month Day, Year)
- Decision notification date
- Deposit/enrollment confirmation deadline
- Financial aid/scholarship application deadlines
- Standardized test score submission deadlines (if separate from application deadline)
- For UK: UCAS application open date, equal consideration deadline, decision deadline, UCAS Extra/Clearing dates if relevant
- For any country: international vs domestic deadlines if different

Search for upcoming intakes: {deadline_hint}. Only include FUTURE deadlines — skip anything before {today}.

FALLBACK: If your first search finds limited deadline data, try these additional searches:
- "{{university}} admissions calendar {{year}}"
- "{{university}} key dates undergraduate {{year}}"
- "{{university}} when to apply undergraduate"
- For UK: "{{university}} UCAS deadline {{year}} entry"

## Scholarships & Financial Aid
Merit-based scholarships (named, with amounts and eligibility), need-based grants, athletic scholarships, international student scholarships, departmental awards, tuition waivers, work-study programs. Include FAFSA/CSS Profile requirements.

## Career Outcomes
Search specifically for each of these — do NOT leave any blank:
- **Average Earnings**: Median/average starting salary for graduates (with source year)
- **Graduation Rate**: Percentage of students who complete the program
- **Job Placement Rate**: Percentage employed within 6 months / 1 year of graduation
- **Top Recruiters**: List specific company names that recruit from this program (e.g., Google, Goldman Sachs, McKinsey, etc.)
- **Prospective Job Roles**: List specific job titles graduates get (e.g., Data Analyst, Software Engineer, Financial Analyst, etc.)
- **Top Industries**: Which industries hire graduates
- **Graduate School Placement**: Percentage who go to graduate/professional school

## Course Insights
Search for specific program-level statistics:
- **Students Admitted Per Year**: Number of students admitted to this specific program annually
- **International Student %**: Percentage of international students in this program
- **Total Program Enrollment**: Total number of students currently enrolled in this program
- **Student-Faculty Ratio**: For this department/program

## Notable Faculty
Search for 3-5 notable professors in this department. For each include:
- Professor name
...
FINAL INSTRUCTIONS:
- Search BROADLY and DEEPLY — at minimum 20-23 separate search queries. Every data point (duration, delivery mode, graduation rate, job roles, study load, tuition, deadlines, SAT/ACT) needs its own dedicated search.
- Return ONLY the markdown content. No preamble, no explanation, no citation blocks.
- The ## headings above are FIXED. Never skip one. If no data, keep the heading and write "Information not available".
- Within each section, use whatever markdown structure (###, bullets, tables, bold) makes the data clearest."""
...
def process_program(university, country, program, dataset, index, total):
    ...
    if dataset in ("phd", "phd_jobs"):
        ...
        prompt = build_prompt_ug(university, country, program)
    else:
    ...
    # Strip any (Source: ...) that Gemini wrote itself BEFORE grounding injection
    source_pattern = re.compile(r'\s*\(Source:\s*https?://[^\)]+\)')
    # Build list of (match_start, match_length) to adjust grounding offsets
    stripped_matches = [(m.start(), len(m.group())) for m in source_pattern.finditer(markdown)]
    markdown = source_pattern.sub('', markdown)

    def adjust_offset(orig_offset):
        """Adjust a grounding offset to account for stripped (Source:) text."""
        removed = 0
        for mstart, mlen in stripped_matches:
            if mstart < orig_offset:
                removed += mlen
            else:
                break
        return orig_offset - removed

    # Inject inline source URLs at each grounding support's end index
    # Process supports in reverse order so insertions don't shift later offsets
    sorted_supports = sorted(supports, key=lambda s: s.get("end", 0), reverse=True)
    for sup in sorted_supports:
        seg_end = adjust_offset(sup.get("end", 0))
        seg_urls = [u for u in sup.get("urls", []) if not _is_blocked_source(u)]
        if not seg_urls or seg_end <= 0 or seg_end > len(markdown):
            continue
        # Use first URL as the primary source
        inline = f" (Source: {seg_urls[0]})"
        markdown = markdown[:seg_end] + inline + markdown[seg_end:]

    # Append master source list
    if all_urls:
        source_block = "\n---\n\n## Sources\n\n"
    ...
    # Append raw grounding data as JSON at the end
    grounding_data = {
        "groundingChunks": [{"index": i, "url": c["url"], "title": c.get("title", "")} for i, c in sorted(result.get("_chunk_urls", {}).items())],
        "groundingSupports": supports,
        "totalSources": len(all_urls),
        "totalSupports": len(supports),
    }
    markdown += f"\n---\n\n## Grounding Data\n\n```json\n{json.dumps(grounding_data, indent=2, ensure_ascii=False)}\n```\n"

    # Count sections found (exclude Sources and Grounding Data from content metrics)
    content_markdown = markdown.split("\n---\n\n## Sources")[0] if "## Sources" in markdown else markdown
    section_headers = re.findall(r'^## (.+)$', content_markdown, re.MULTILINE)
    verified = len(re.findall(r'status:\s*verified', content_markdown))
    missing_count = content_markdown.lower().count("information not available")

    # Count empty sections: ## headers immediately followed by "Information not available"
    empty_sections = len(re.findall(r'^## .+\n+Information not available', content_markdown, re.MULTILINE))
    total_content_sections = len([h for h in section_headers if h not in ("Sources", "Grounding Data")])
    ...
    # Measure content size (before Sources/Grounding JSON padding)
    content_size = len(content_markdown.encode("utf-8"))

    print(
        f"  DONE {label} — {elapsed:.0f}s — "
        f"{len(section_headers)} sections ({empty_sections} empty), "
        f"{verified} cited / {missing_count} gaps, "
        f"{len(all_urls)} sources, {content_size//1024}KB content"
    )

    # Quality check: flag low-quality files for retry
    # Bad if: >15 gaps, OR content <5KB, OR more than half sections are empty
    quality_ok = (missing_count <= 15
                  and content_size >= 5000
                  and empty_sections <= total_content_sections // 2)
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if level in ("phd", "doctorate"):
                dataset = "phd"
            elif level in ("bachelor", "undergraduate", "ug"):
                dataset = "ug"
            else:
                dataset = "masters"
...
def main():
    ...
    def handle_result(item, result):
        ...
        with _progress_lock:
            ...
            if result.get("status") == "success":
                _success_count[0] += 1
                # Flag low-quality successes for retry
                if not result.get("quality_ok", True):
                    low_quality_items.append((item, result))
            else:
                _error_count[0] += 1
    ...
    # ── Quality retry: re-scrape low-quality files (>15 gaps or <5KB) ───
    if low_quality_items:
        print(f"\n{'─'*60}")
        print(f"QUALITY RETRY: {len(low_quality_items)} low-quality files (>15 gaps or <5KB)")
        print(f"{'─'*60}\n")
        time.sleep(5)

        quality_retry_results = []
        with ThreadPoolExecutor(max_workers=min(args.workers, 5)) as executor:
            futures = {}
            for item, prev_result in low_quality_items:
                future = executor.submit(
                    process_program,
                    item["university"],
                    item["country"],
            ...
            for future in as_completed(futures):
                ...
                try:
                    result = future.result()
                    result["program_id"] = item["program"].get("program_id", "")
                    quality_retry_results.append(result)
                    append_progress(run_id, {**result, "quality_retry": True})

                    # Only replace if new version is better
                    if result.get("status") == "success" and result.get("quality_ok", False):
                        for i, r in enumerate(results):
                            if (r.get("university") == result["university"]
                                    and r.get("program") == result["program"]):
                                results[i] = result
                                break
                        print(f"  QUALITY-OK {result['university']} — {result['program']} "
                              f"({result.get('missing_fields', '?')} gaps, {result.get('file_size', 0)//1024}KB)")
                    elif result.get("status") == "success":
                        # Retry succeeded but still low quality — keep whichever is better
                        # Compare both gaps AND content size to avoid replacing good files with empty ones
                        new_gaps = result.get("missing_fields", 999)
                        old_gaps = prev_result.get("missing_fields", 999)
                        new_size = result.get("file_size", 0)
                        old_size = prev_result.get("file_size", 0)
                        if new_gaps < old_gaps and new_size >= old_size * 0.5:
                            for i, r in enumerate(results):
                                if (r.get("university") == result["university"]
                                        and r.get("program") == result["program"]):
                                    results[i] = result
                                    break
                            print(f"  QUALITY-IMPROVED {result['university']} — {result['program']} "
                                  f"({old_gaps}->{new_gaps} gaps)")
                        else:
                            print(f"  QUALITY-KEPT-OLD {result['university']} — {result['program']} "
                                  f"(retry {new_gaps} gaps vs orig {old_gaps})")
                    else:
                        print(f"  QUALITY-RETRY-FAIL {result.get('university', '?')} — {result.get('status', '?')}")
                except Exception as e:
                    print(f"  QUALITY-RETRY-EXCEPTION {item['university']} — {e}")

        quality_improved = sum(1 for r in quality_retry_results
                              if r.get("status") == "success" and r.get("quality_ok", False))
        print(f"\n  Quality retry: {quality_improved}/{len(low_quality_items)} now meet quality threshold")
