#!/usr/bin/env python3
"""
merge_avg_patch.py — Merge Gemini patch values into the original AVG volunteering files.

For each of the 30 AVG-tier volunteering files, replaces 5 fields with values
...
Input:
  - originals: volunteering_data/extracted/avg/*.json (30 files)
  - patches:   volunteering_data/patch_avg_weak_gemini3/*.json (30 files)

Output:
  - merged: volunteering_data/extracted/avg_patched/*.json (30 files)
...
ORIG_DIR = os.path.join(SCRIPT_DIR, "volunteering_data", "extracted", "avg")
PATCH_DIR = os.path.join(SCRIPT_DIR, "volunteering_data", "patch_avg_weak_gemini3")
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "volunteering_data", "extracted", "avg_patched")

# The 5 fields we replace from the patch
REPLACE_FIELDS = [
    "mode",
    "eligibility_text",
    "minimum_hours",
...
def main():
    parser = argparse.ArgumentParser(description="Merge Gemini patch into AVG volunteering originals")
    ...
    for fname in orig_files:
        ...
        if not os.path.exists(patch_path):
            ...
            missing_patch += 1
            if not args.dry_run:
                # Still copy original to keep output folder complete
